...
export const formatMoney = (num: number, fix = 2, includeZeros = false) => {
  ...
  const formattedNum = Number(x.toFixed(fix)).toLocaleString("en-US")

  return units[Math.floor(unit / 3) - 2]
    ? formatNumber(formattedNum, includeZeros) + units[Math.floor(unit / 3) - 2]
...
export const addCommas = (num: number ) => {
  ...
  const formattedNum = parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, ",") + (parts[1] ? "." + parts[1] : "");
  return formattedNum;
